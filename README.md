...
- 支持多条背景音乐按时间点进入、按时长截断、淡入淡出
- 支持基于语音时间的背景音乐自动 ducking
...
- `examples/basic.json`
  ...
  - 带多人声、语气变化、词级颜色、不同字号、背景音乐和 ducking
...
- `examples/rubber-duck.json`
  - “小黄鸭调试”风格示例
  - 适合看长短句混排和更明显的角色变化
- `examples/lyrics-demo.lrc`
  - 歌词时间轴源文件
  - 用来生成标准 GlyphFall JSON 配置
- `examples/lyrics-demo.json`
  - 由 LRC 解析脚本生成的标准示例配置
  - 生成后仍然可以像其他 example 一样继续手改
...
- `examples/standup-roast.json`
  - “晨会群口吐槽”风格示例
  - 适合看更密集的对白切换和短句推进
- `examples/twinkle-demo.lrc`
  - `Twinkle Twinkle Little Star` 的歌词时间轴源文件
- `examples/twinkle-demo.json`
  - 基于真实歌曲音频生成的标准示例配置
  - 适合直接预览“真歌 + 歌词滚动”的成片方向
...
```ts
{
  ...
  timings: [
    ...
    {startMs: 2200, endMs: 4100},
    {startMs: 4100, endMs: 6300},
  ],
  captions: [
...
- `timings`: 直接提供的字幕时间轴；如果存在，就不强制走语音生成
- `backgroundMusic`: 背景音乐轨道数组，支持多条、支持按时间点开始和按时长截断
- `audioMix`: 音频混音配置，当前支持背景音乐 ducking
- `debug`: 调试开关
- `visuals`: 字体和字幕视觉配置
- `effects`: 当前字幕视觉特效配置
...
- `endMs`: 可选，结束时间，毫秒
- `durationMs`: 可选，时长，毫秒
- `durationSeconds`: 可选，时长，秒

`backgroundMusic` 每个条目可选项：

- `src`: 必填，音频路径；相对路径会按 `public/` 目录解析
- `startMs`: 可选，单位毫秒；这条背景音乐从视频的哪个时间点开始播放
- `startFrame`: 可选，单位帧；优先级高于 `startMs`
- `durationMs`: 可选，单位毫秒；播放多长时间后停止
- `durationFrames`: 可选，单位帧；优先级高于 `durationMs`
- `volume`: 可选，音量倍率，默认 `1`
- `playbackRate`: 可选，播放倍率，默认 `1`
- `fadeInMs`: 可选，淡入时长，单位毫秒
- `fadeInFrames`: 可选，淡入时长，单位帧；优先级高于 `fadeInMs`
- `fadeOutMs`: 可选，淡出时长，单位毫秒
- `fadeOutFrames`: 可选，淡出时长，单位帧；优先级高于 `fadeOutMs`

`audioMix` 可选项：

- `ducking`: 背景音乐自动 ducking 配置

`audioMix.ducking` 可选项：

- `enabled`: 是否启用
- `volumeMultiplier`: 有人声时背景音乐的压低倍率
- `attackMs`: 进入 ducking 的时长，单位毫秒
- `attackFrames`: 进入 ducking 的时长，单位帧；优先级高于 `attackMs`
- `releaseMs`: 退出 ducking 的时长，单位毫秒
- `releaseFrames`: 退出 ducking 的时长，单位帧；优先级高于 `releaseMs`
...
| `outputVideoName` | `string` | 否 | 输出视频名 |
| `timings` | `object[]` | 否 | 直接提供的字幕时间轴 |
| `backgroundMusic` | `object[]` | 否 | 背景音乐轨道数组 |
| `audioMix` | `object` | 否 | 音频混音配置 |
| `debug` | `object` | 否 | 调试开关 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `startFrame` | `number` | 否 | 起始帧 |
| `endFrame` | `number` | 否 | 结束帧 |
| `startMs` | `number` | 否 | 起始时间，毫秒 |
| `endMs` | `number` | 否 | 结束时间，毫秒 |
| `durationMs` | `number` | 否 | 时长，毫秒 |
| `durationSeconds` | `number` | 否 | 时长，秒 |

`backgroundMusic[*]`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `src` | `string` | 是 | 音频路径；相对路径按 `public/` 解析 |
| `startMs` | `number` | 否 | 开始播放时间，毫秒 |
| `startFrame` | `number` | 否 | 开始播放时间，帧；优先于 `startMs` |
| `durationMs` | `number` | 否 | 播放时长，毫秒 |
| `durationFrames` | `number` | 否 | 播放时长，帧；优先于 `durationMs` |
| `volume` | `number` | 否 | 音量倍率，默认 `1` |
| `playbackRate` | `number` | 否 | 播放倍率，默认 `1` |
| `fadeInMs` | `number` | 否 | 淡入时长，毫秒 |
| `fadeInFrames` | `number` | 否 | 淡入时长，帧；优先于 `fadeInMs` |
| `fadeOutMs` | `number` | 否 | 淡出时长，毫秒 |
| `fadeOutFrames` | `number` | 否 | 淡出时长，帧；优先于 `fadeOutMs` |

`audioMix`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `ducking` | `object` | 否 | 背景音乐 ducking 配置 |

`audioMix.ducking`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `enabled` | `boolean` | 否 | 是否启用自动 ducking |
| `volumeMultiplier` | `number` | 否 | 有人声时背景音乐的压低倍率 |
| `attackMs` | `number` | 否 | 进入 ducking 的时长，毫秒 |
| `attackFrames` | `number` | 否 | 进入 ducking 的时长，帧；优先于 `attackMs` |
| `releaseMs` | `number` | 否 | 退出 ducking 的时长，毫秒 |
| `releaseFrames` | `number` | 否 | 退出 ducking 的时长，帧；优先于 `releaseMs` |
...
- `backgroundMusic[*].src` 推荐使用 `public/` 下的相对路径，例如 `music/intro-loop.mp3`
- `startFrame` 优先级高于 `startMs`
- `durationFrames` 优先级高于 `durationMs`
- `fadeInFrames` 优先级高于 `fadeInMs`
- `fadeOutFrames` 优先级高于 `fadeOutMs`
- 如果背景音乐条目没有配置播放时长，会从配置的开始时间点一直播放到视频结束
- `audioMix.ducking` 会基于语音分段时间自动压低背景音乐，不会改动旁白本身的音量
- 如果顶层已经直接提供 `timings`，`npm run render` 会跳过自动语音生成
...
Studio 里现在每个 example 都是单独的 composition，可以直接在左侧列表里选择，例如：

- `GlyphFallBasic`
- `GlyphFallDeployFriday`
- `GlyphFallLyricsDemo`
- `GlyphFallRubberDuck`
- `GlyphFallStandupRoast`
- `GlyphFallTechLaunch`
- `GlyphFallTwinkleDemo`
...
这样在 Studio 左侧切不同 example composition 时，就不会因为某个 example 还没生成 preview 缓存而只剩画面没有声音。

如果你要做“音乐 + 歌词滚动”的 demo，推荐先把 LRC 解析成标准 GlyphFall 配置，再在生成结果上继续手改。流程是：

```bash
npm run lyrics:parse -- --input examples/lyrics-demo.lrc --output examples/lyrics-demo.json --audio music/game-bgm.ogg
npm run render -- --input examples/lyrics-demo.json
```

第一步产出的 `examples/lyrics-demo.json` 仍然是普通的 GlyphFall 配置文件，包含 `captions`、`utterances`、`timings`、`layoutMap`、`visuals`、`effects`、`backgroundMusic` 等字段。后续你可以像编辑其他 example 一样继续改它。

如果你想直接看一版“真歌 + 歌词滚动”的 demo，可以直接渲染：

```bash
npm run render -- --input examples/twinkle-demo.json
```

对应音频是 [twinkle-twinkle-full-lyrics.ogg](/home/heyicong/.openclaw/workspace/projects/subtitle-feed/public/music/twinkle-twinkle-full-lyrics.ogg)，来源和授权在 [ATTRIBUTION.md](/home/heyicong/.openclaw/workspace/projects/subtitle-feed/public/music/ATTRIBUTION.md)。

如果你要在 Studio 里调试一份本地 JSON 输入，而不是每次都 `render`，可以先把它预处理成浏览器可读的清单：

```bash
npm run studio:prepare -- --input build/local-midnight-lament-demo.json
npm run studio
```

然后在 Studio 左侧直接选择 `GlyphFallPreparedInput` 这个 composition。

这条链路会把本地 JSON 里的外部音频复制到 `public/studio-input/assets/`，并在 `public/studio-input/current.json` 里记录相对路径，方便在 Studio 里直接调试，不需要把音频拷进仓库。

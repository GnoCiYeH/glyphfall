...
当前推荐的唯一输入入口是 [src/data/subtitles.json](/home/heyicong/.openclaw/workspace/projects/subtitle-feed/src/data/subtitles.json)。字幕内容、画面参数、调试开关、字体、位移配置、语音参数都在这个文件里维护。
...
- `src/data/subtitles.json`
  - 唯一人工输入与配置入口
  - 包含字幕内容、画面参数、调试开关、视觉配置、位移配置、语音参数
...
`src/data/subtitles.json` 目前是唯一推荐入口。顶层可选项如下：
...
- `audioSrc`: 输出音频在 `public/` 下的相对路径，默认 `audio/narration.mp3`
- `voice`: 全局默认人声
- `rate`: 全局默认语速，例如 `+0%`
- `pitch`: 全局默认音高，例如 `+0Hz`
- `volume`: 全局默认音量，例如 `+0%`
...
```json
{
  ...
  "chunking": {
    ...
    "breakOnPause": true,
    "pauseThresholdMs": 320,
    "mergeShortTail": true
  },
  "utterances": [
    {
      "id": "u1",
      "text": "hello 大家好，我是小派，现在为大家展示 GlyphFall。",
      "voice": "zh-CN-XiaoxiaoNeural"
    },
    {
      "id": "u2",
  ...
  "captions": [
    ...
    },
    {
      ...
      "layoutKey": "cw",
      "tokens": [
        {"text": "我是", "color": "#ffffff"},
        {"text": "小派", "color": "#fb7185"}
      ]
    },
    {
      "id": "c3",
      "text": "现在为大家展示 GlyphFall",
      "utteranceId": "u1",
      "layoutKey": "up",
      "tokens": [
        {"text": "现在为大家展示 ", "color": "#ffffff"},
        {"text": "GlyphFall", "color": "#a3e635"}
      ]
    },
    {
      "id": "c4",
    ...
    },
    {
      ...
      "fontSize": 108,
      "tokens": [
        {"text": "旋转", "color": "#f97316"},
        {"text": "瀑布", "color": "#38bdf8"}
      ]
    }
  ]
}
...
| `audioSrc` | `string` | 否 | 输出音频在 `public/` 下的相对路径 |
...
2. 运行 `npm run render` 时，脚本会自动检查语音资产是否需要更新
3. 如需更新，脚本会先用 Edge TTS 生成 `public/audio/narration.mp3`
4. 再用 Whisper 拉词级时间，输出 `src/data/generated-speech.json`
5. 然后继续执行 Remotion 渲染
...
如果要接音频文件，把文件放到 `public/audio/`，然后在 `subtitles.json` 的 `audioSrc` 中填相对路径，比如 `audio/narration.mp3`。
...
- 触发条件是：
  - `src/data/generated-speech.json` 不存在
  - `public/audio/narration.mp3` 不存在
  - `src/data/subtitles.json` 的 hash 与 `generated-speech.json` 中记录的 `meta.inputHash` 不一致
